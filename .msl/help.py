...
def writecsv(fn, data, chperm = True): #For List of (Row List)
	fd = open(fn, 'wb');
	csvw = csv.writer(fd);
	mappl(lambda i: csvw.writerow(i), data);
	fd.close();
	if(chperm):
		os.chmod(fn, 0777);

def writecsv1(fn, data, mapping={}, order=[]): #For list of (Row Dict)
	if(len(data) == 0):
		writecsv(fn, [['']]);
	else:
		keys = sortbypref(data[0].keys(), order);
		writecsv(fn, [mappl(lambda x: g(mapping, x, x), keys)]+mappl(lambda row: pkey1(row, keys).values(), data));
...
def sortbypref(inp, s=[]):
	inpi = range(len(inp));
	inpi.sort(lambda x,y: s.index(inp[x]).__cmp__(s.index(inp[y])) if ((inp[x] in s) and (inp[y] in s)) else x.__cmp__(y) );
	return pkey1(inp, inpi).values();
